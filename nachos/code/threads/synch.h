...
class Semaphore {
  public:
    ...
    ~Semaphore(); 
    Semaphore(char* debugName, int initialValue, int key);    					// de-allocate semaphore
    char* getName() { return name;}			// debugging assist
    
    void P();	 // these are the only operations on a semaphore
    void V();	 // they are both *atomic*

    void semValueSet(int *val);
    void semValueReturn(int *val);    
    int key;
  private:
...
class Condition {
  public:
    Condition(char* debugName, int gkey);		// initialize condition to 
					// "no one waiting"
