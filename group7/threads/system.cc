...
int semNextId,condNextId, pageFaults;
Semaphore *semaphoreList[1000];
Condition *condList[1000];
TimeSortedWaitQueue *sleepQueueHead;	// Needed to implement SC_Sleep
Inverted invertTable[NumPhysPages];
int schedulingAlgo, replaceAlgo;			// Scheduling algorithm to simulate
...
int semAllocate(int gkey)
{
    for(int i=0; i<semNextId; i++)
    {
        if(semaphoreList[i]!=NULL)
        {
            if(semaphoreList[i]->key==gkey)
                return i;
        }
    }
    Semaphore *newsem= new Semaphore("arihantKumar",0,gkey);
    semaphoreList[semNextId++]=newsem;
    return (semNextId-1);
}

int condAllocate(int gkey)
{
    for(int i=0; i<condNextId; i++)
    {
        if(condList[i]!=NULL)
        {
            if(condList[i]->key==gkey)
            return i;
        }
    }
    Condition *newcond= new Condition("arihantKumar",gkey);
    condList[condNextId++]=newcond;
    return (condNextId-1);
}
...
Initialize(int argc, char **argv)
{
    ...
    for (i=0; i<MAX_BATCH_SIZE; i++) {
      ...
      {
          invertTable[i].thread = NULL;
          invertTable[i].free = FALSE;
          invertTable[i].vpn = 0;
          invertTable[i].shared = FALSE;
          invertTable[i].numShared = 0;
          invertTable[i].fifo = 0;
	  invertTable[i].lru = 0;
	  invertTable[i].fork = FALSE;
